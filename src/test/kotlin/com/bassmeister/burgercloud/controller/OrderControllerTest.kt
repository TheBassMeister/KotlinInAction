...
class OrderControllerTest(
    ...
    fun `Create New Order`() {
        val order = Mono.just(createTestOrder())
        testClient.post().uri(orders).contentType(MediaType.APPLICATION_JSON)
            .body(order, Order::class.java).exchange()
    ...
    @Test
    fun `Cancel Order`() {
        var order = Mono.just(createTestOrder())
        var orderId = 0

        testClient.post().uri(orders).contentType(MediaType.APPLICATION_JSON)
            .body(order, Customer::class.java).exchange().expectBody().jsonPath("$.id").value<Int> {
                orderId = it
            }
